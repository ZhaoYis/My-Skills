...
description: Global steps 17–22 (pre-commit 5a/5b, decision 5 commit, 5c push, decision 6 merge, post-merge, final summary). Requires phase-5-unit-tests.md step 16 first; step 22 ends the main pipeline.
compatibility: Requires git and reachable remotes for push/pull; AskQuestion recommended in Cursor.
---

## Phase 6: Commit, merge, and push
...
18. **Stage and commit**
    ...
    **[Decision 5] Options:**
    ...
    - `Cancel commit` — do not commit; show resume guidance (user may **resume from Phase 5 `phase-5-unit-tests.md` Step 16 or Phase 6 Step 17** for unit-test gate, staging, commit, and push)
...
19. **Push**
    ...
    **If push fails**, **AskQuestion**:
    ...
    - `Terminate pipeline` — exit (note: if a commit exists locally, run `git push origin <branch>` later — same flow as §19)

20. **[Decision 6] Merge (only if decision 4 was commit-and-merge)**
    ...
    **Pre-merge self-check** (inlined merge flow):
    ...
    - If `origin/<source-branch>` exists: **prefer** `git log origin/<source-branch>..HEAD --oneline` empty before merging; if there are unpushed commits, **AskQuestion** whether to finish §19 first or proceed (explain risk).
    - Prefer a successful §19 push before merging; if only network blocked the push, fix push or confirm with user before merging.
...
22. **Final summary**
    ...
    - Phase table (propose / apply / review / archive / **unit tests** / commit / merge)
    - **Decision 4b** outcome (tests added / skipped) and, when relevant, command + pass/fail recap
