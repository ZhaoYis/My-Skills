...
describe('end-to-end pipeline lifecycle (mocked openspec CLI)', () => {
  ...
  it('runs a trivial change lifecycle after runInit', async () => {
    ...
    const init = await state(repo, 'init', 'fix-typo', 'feature/fix-typo', '--route', 'trivial');
    expect(init.code).toBe(0);

    // Trivial route skips Phase 1 and therefore does not require proposal approval.
  ...
  it('runs a standard change lifecycle: propose → apply → archive → push', async () => {
    ...
    await state(repo, 'init', 'add-feature', 'feature/add-feature', '--route', 'standard');
    ...
    await state(repo, 'set', 'add-feature', 'delivery.commitSha', 'abc123');
    await state(repo, 'set', 'add-feature', 'delivery.sourcePushed', 'true');
