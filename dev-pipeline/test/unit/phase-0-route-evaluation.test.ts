...
describe('Phase 0 Template Route Evaluation', () => {
  it('includes route evaluation section in template', () => {
    ...
    expect(templateContent).toContain('在确定入口类型后、进入具体 Phase 前，必须评估变更的风险等级并选择合适的 route');
  });
  ...
  it('includes route decision flow with askTool', () => {
    ...
    expect(templateContent).toContain('**question**：`根据需求分析，推荐使用 <recommended-route> route');
  });
  ...
  it('includes route selection recording command', () => {
    ...
    expect(templateContent).toContain('node <SKILL_ROOT>/scripts/dev-pipeline-state.mjs decision "<name>" route_choice "<route>"');
  });
  ...
  it('includes skip conditions for existing changes', () => {
    ...
    expect(templateContent).toContain('如果状态文件中已存在 `route.choice` 字段（续接已有 change），跳过 Route 评估');
  });
  ...
  it('includes phase execution matrix for each route', () => {
    ...
    expect(templateContent).toContain('如果用户选择 `trivial` route，后续流程将跳过 Phase 1（提案）、Phase 3（审查）、Phase 4（单测）、Phase 5（归档）、Phase 7（合并）');
    expect(templateContent).toContain('如果用户选择 `standard` route，后续流程将跳过 Phase 3（审查）、Phase 4（单测）、Phase 7（合并）');
