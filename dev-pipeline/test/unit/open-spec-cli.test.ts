...
describe('resolveOpenSpecInvocation', () => {
  it.each([
    ['--version'],
    ['init', '--tools', 'claude'],
  ])('runs openspec %s through cmd.exe with PATHEXT resolution on Windows', (...args) => {
  ...
  it('recognizes command-not-found errors on each platform', () => {
    ...
    expect(isOpenSpecCliMissingError({ code: 'ENOENT' }, 'win32')).toBe(true);
    expect(
      isOpenSpecCliMissingError(
        {
          code: 1,
    ...
    expect(
      isOpenSpecCliMissingError(
        { code: '1', stderr: "'openspec' 不是内部或外部命令，也不是可运行的程序或批处理文件。" },
        'win32',
      ),
    ).toBe(true);
  });
  ...
  it('does not treat other command failures as a missing CLI', () => {
    ...
    expect(isOpenSpecCliMissingError({ code: 1 }, 'win32')).toBe(false);
    expect(isOpenSpecCliMissingError({ code: 2 }, 'win32')).toBe(false);
    expect(
      isOpenSpecCliMissingError({ code: 1, stderr: 'openspec: simulated failure' }, 'win32'),
    ).toBe(false);
