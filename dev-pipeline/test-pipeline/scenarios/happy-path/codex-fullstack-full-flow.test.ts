...
describe('E2E - Full gated delivery (Codex)', () => {
  ...
  it('installs the Codex skill bundle and records completed delivery state', async () => {
    ...
    expect(await fs.pathExists(path.join(env.rootDir, '.codex/prompts/opsx-dev-pipeline.md'))).toBe(true);
    expect(await fs.pathExists(path.join(env.rootDir, '.codex/commands/opsx-dev-pipeline.md'))).toBe(true);
