...
const scenario: ScenarioConfig = {
  name: 'backend-only-delivery',
  sampleProject: 'backend-only',
  phases: ALL_PHASES,
  toolId: 'claude',
  openspecMode: 'mock',
  changeName: 'add-item-tags',
  featureDescription: 'Add an optional tags field to the Item model',
};

describe('E2E - Backend-only gated delivery', () => {
  ...
  it('executes all seven phases without synthetic or skipped results', () => {
  ...
  it('installs the current skill bundle and records completed delivery state', async () => {
    expect(await fs.pathExists(path.join(env.skillRoot, 'SKILL.md'))).toBe(true);
    expect(await fs.pathExists(path.join(env.skillRoot, 'agents/openai.yaml'))).toBe(true);
    ...
    const state = await fs.readJson(
      path.join(env.rootDir, 'openspec/.pipeline-state/add-item-tags.json'),
    );
  ...
  it('generates schema-valid JSON and Markdown reports from real results', async () => {
