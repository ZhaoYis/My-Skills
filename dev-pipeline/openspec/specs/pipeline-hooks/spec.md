...
定义 opsx-dev-pipeline 的流水线宿主管控层（Pipeline Hooks），把横切且高风险的约束（危险 Bash 命令、敏感文件写入）从提示词层下沉到宿主 hook 层强制执行。覆盖 Claude Code 和 OpenCode 的自动生成场景，并为 Cursor / Codex 提供手动配置指引。

## Requirements

### Requirement: 通用危险 Bash 命令拦截脚本

系统 MUST 提供 `scripts/hooks/block-dangerous-bash.mjs` 脚本，作为 Claude Code `PreToolUse(Bash)` hook 和 OpenCode `tool.execute.before(Bash)` hook 共用的命令拦截实现。
...
系统 MUST 提供 `scripts/hooks/block-sensitive-write.mjs`，作为 `PreToolUse(Write|Edit)` hook 共用实现。
...
- `PreToolUse` 事件，matcher `Bash`，hooks 数组引用 `<SKILL_ROOT>/scripts/hooks/block-dangerous-bash.mjs`（SKILL_ROOT 通过 Handlebars 变量注入，运行时为绝对路径或相对仓库根的 `node_modules/.bin/...` 路径）
- `PreToolUse` 事件，matcher `Write|Edit|MultiEdit`，hooks 数组引用 `block-sensitive-write.mjs`
...
- **THEN** 生成的 `.claude/settings.json` MUST 包含两条 PreToolUse hook 条目，分别指向 `block-dangerous-bash.mjs` 和 `block-sensitive-write.mjs`
...
- `hooks.PreToolUse` 数组，matcher `Bash` 引用 `block-dangerous-bash.mjs`
- `hooks.PreToolUse` 数组，matcher `Write|Edit|MultiEdit`（OpenCode 工具名） 引用 `block-sensitive-write.mjs`
...
脚本 MUST 使用 `JSON.parse` 解析，兼容两种 tool_name 大小写，并支持 `MultiEdit` / `edit` / `Edit` 三种 Write 工具变体。

#### Scenario: 脚本同时被 Claude 和 OpenCode 调用

- **WHEN** 同一份 `block-dangerous-bash.mjs` 被 Claude Code 以 `tool_name: "Bash"` 调用，又被 OpenCode 以 `tool_name: "bash"` 调用
...
- `JSON.parse` 失败 MUST 视为「无法判断」，放行（exit 0），并在 stderr 输出 WARN 日志（不阻断流水线）
