...
| 顺序  | 入口                  | 类型    | 用途摘要                                                   | 备注                                        |
| --- | ------------------- | ----- | ------------------------------------------------------ | ----------------------------------------- |
| 0   | `opsx-learn`        | skill | 渐进式知识沉淀：功能、接口、规则、FAQ、术语、配置约束、故障案例等                     | 持续进行                                       |
...
- 三套工具 overlay（`CLAUDE.md` / Cursor rule / Codex prompt）提供各自工具的项目级配置入口。
...
| `upgrade`    | `npx opsx-dev-pipeline upgrade --dry-run`        | 在 sync 基础上额外采纳包内**新增** skill/command；执行前会对比 manifest 与 CLI 版本，manifest 偏新或无法解析时需确认（`--yes` 跳过） |
