...
| `agent-run` | `npx opsx-dev-pipeline agent-run <change> --planner model --endpoint <url> --model <name>` | 使用 OpenAI-compatible 模型 Planner；API Key 从 `OPSX_AGENT_API_KEY` 读取 |
...
模型 Planner 的 API Key 只从环境变量读取：`OPSX_AGENT_API_KEY`；也可用
`OPSX_AGENT_ENDPOINT` 和 `OPSX_AGENT_MODEL` 代替 `agent-run` 中的 endpoint/model 参数。
