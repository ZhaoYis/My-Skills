...
path_hints:
  - "openspec/**"
description: "合并与交付阶段的详细指引，包括目标分支合并、推送验证、标签创建、分支清理"
---
...
- merge、target push、删除本地分支、删除远程分支、创建标签和推送标签分别确认，禁止合并确认。
- 禁止 `git push --force`、`git branch -D`、未经逐文件确认的全局 ours/theirs。
- 任一步失败时先记录 `pause` 和失败事实，不得谎报完成或自动选择破坏性恢复方案。

> **前置条件**：本 Phase 仅在 `postArchiveAction=merge` 且 Phase 6 Step22 源分支推送完成后执行。
> 进入前必须通过 Phase 7 门禁：`postArchiveAction=merge`、`delivery.commitSha` 已记录、`delivery.sourcePushed=true`。
...
只有 target push 完成时才执行 `complete`。恢复执行时先读取状态：
...
只有状态已经是 `completed`、commit message 包含 `finalize pipeline delivery state`，且 `git diff --quiet` 返回 0 时，才说明最终状态已完整提交，可跳过 25.1 和 25.2。否则继续执行；不要先重复调用 `complete`，以免仅因刷新 `updatedAt` 产生多余提交。

**标签准备**：在 `complete` 前询问是否创建标签。选择标签时先校验名称与不存在性，展示将要标记的 target HEAD，并单独询问 `确认创建标签` / `不创建标签` / `终止流程`。确认创建后先记录标签名；实际 `git tag` 必须等 25.2 的状态提交和 25.3 的 target push 完成后，回到 Step26 执行：
...
> ⚠️ 请在本步骤（提交状态 + push）完成后再执行 Step26 创建 tag，确保 tag 覆盖完整的流水线状态文件。
...
1. **幂等性检查**——确认状态文件是否已被提交：
   ...
   如果返回的 commit message 包含 `finalize pipeline delivery state`，且 `git diff --quiet` 返回 0，说明状态已提交过 → 跳过 25.2，直接进入 25.3。
...
6. 提交：
   ...
   ```bash
   # 将 <change-name> 替换为当前 change 的实际名称
...
推送最终状态到 target branch：

询问 `推送最终状态到 target branch` / `终止流程`。确认后：

```bash
git push origin "<target-branch>"
```

推送失败不得自动 rebase。先 fetch 并展示 ahead/behind，再询问 `pull --rebase`、保持本地等待人工处理或终止。若最终无法推送成功，调用 `pause "<name>" "state-commit-push-failed"`。

推送成功后进入 Step26；标签处理结束后回到本节继续最终确认和摘要。
...
**展示摘要**：change、归档路径、审查报告、测试/verify 结果、各决策、提交 SHA、source/target push、合并策略、分支清理和标签结果。跳过项明确标记，不得用"完成"掩盖暂停或失败。

## Step26：分支清理与标签

> ⚠️ 请确认 Step25 的状态提交与推送已完成，再继续本步骤创建 tag。确保 tag 覆盖完整的流水线状态文件。

先验证 source 已包含在远程 target：
...
标签必须基于 target HEAD。读取 Step25.1 已确认并写入 `delivery.tag` 的标签名：未记录标签名时明确跳过；已记录时先校验标签不存在并展示目标提交，再执行已获确认的创建动作，并单独确认是否推送：
