...
1. `schema = custom` 时，优先复用 **Phase 2 步骤 5** 已获取的 `dev-pipeline-change-context.sh` 结果，并结合元数据推导测试/验证路径；仅在该结果缺失或元数据刚变更时，才重新执行 `bash <SKILL_ROOT>/scripts/dev-pipeline-change-context.sh "<name>"`
