...
仅当 **步骤 c** 已选「确认提案，开始实施」后：对 change **`fix-cr-<type>`** 执行 `phase-2-apply.md` **步骤 5–7**（`dev-pipeline-instructions-apply.sh "<name>"` / 等价 `openspec instructions apply`），按 `tasks.md` 勾选任务；**禁止**跳过 **步骤 b–c** 直接改业务代码。
...
```bash
bash <SKILL_ROOT>/scripts/dev-pipeline-archive.sh "fix-cr-<type>" -y --skip-specs
```
