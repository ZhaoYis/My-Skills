...
4. Reuse an `in-progress` Phase 5 entry executed by `openspec-sync-specs` when present; otherwise start the sync audit entry:
   ```bash
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs record-phase "<name>" 5 17 openspec-sync-specs --start
   ```

### Execute

Load and follow `.cursor/rules/openspec-sync-specs/SKILL.md` completely. Preserve its intelligent merge semantics, idempotency, change path resolution, and summary requirements.
...
1. Complete the audit record:
   ```bash
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs record-phase "<name>" 5 17 openspec-sync-specs
   ```
