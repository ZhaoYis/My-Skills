...
2. Run:
   ```bash
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs get "<name>"
   ```
...
4. If state is missing, **MUST call AskQuestion and wait for an explicit choice**: `Associate external requirement` / `Skip association`. This gate is mandatory even when the request is detailed or the user says to proceed; never infer a skip from context. If the user chooses association, collect `featureId` and optional `featureUrl`. Then run exactly one `init` branch, followed by the remaining commands, and check every exit code:
   ```bash
   ...
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs transition "<name>" 1 3
   ```
...
6. Check `phaseHistory` for an `in-progress` Phase 1 entry executed by `openspec-propose`. Reuse it when present; otherwise start the audit entry below. Starting it also changes an existing `pipeline` state to `hybrid`:
   ```bash
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs record-phase "<name>" 1 3 openspec-propose --start
   ```

### Execute

Load and follow the complete instructions in `.cursor/rules/openspec-propose/SKILL.md`. Keep that Skill as the source of truth for artifact creation, dependency order, store selection, clarification, and output paths.
...
1. `record-phase`, because later recovery and inference depend on `phaseHistory`:
   ```bash
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs record-phase "<name>" 1 5 openspec-propose
   ```
2. `decision`, after the phase record is durable:
   ```bash
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs decision "<name>" requirementsConfirmed true
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs decision "<name>" proposalApproved true
   ```
3. `transition`, after `proposalApproved` is durable:
   ```bash
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs transition "<name>" 2 6
   ```
4. Consistency check:
   ```bash
   node .cursor/rules/opsx-dev-pipeline/scripts/dev-pipeline-state.mjs get "<name>"
