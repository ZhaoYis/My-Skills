...
path_hints:
  - "openspec/changes/**"
description: "入口判断与 Route 分级，包括环境预检、需求关联、Route 选择"
---

# Phase0: 入口判断 + Route 分级
...
在初始化状态前，必须根据变更复杂度确定 Route 类型：

| Route | 适用场景 | Phase 路径 | 跳过阶段 |
|-------|---------|-----------|---------|
| `trivial` | typo 修复、配置调整、单文件改动、文档更新 | 0 → 2 → 6 | 1, 3, 4, 5, 7 |
| `standard` | 常规功能开发、Bug 修复、小范围重构 | 0 → 1 → 2 → 3 → 4 → 5 → 6 → 7 | 无 |
| `full` | 跨模块重构、架构变更、高风险变更 | 0 → 1 → 2 → 3 → 4 → 5 → 6 → 7 | 无（增强验证） |

使用 **AskUserQuestion** 询问用户选择 Route：
- **header**：`Route 分级`
- **question**：`根据变更复杂度选择合适的 Route。Route 决定了流水线经过的 Phase 路径。`
- **options**：
  - `trivial — 简单变更` — typo 修复、配置调整、单文件改动。路径：Phase0 → Phase2 → Phase6
  - `standard — 标准变更（推荐）` — 常规功能开发、Bug 修复。路径：完整 7 Phase 流程
  - `full — 复杂变更` — 跨模块重构、架构变更。路径：完整 7 Phase 流程，增加更多验证
...
2. 用户在对话框中输入需求信息（如 featureId、featureUrl 等），从用户输入中提取信息，根据是否提供 URL 只执行下面一条命令：
   ```bash
   # 未提供 URL
   node <SKILL_ROOT>/scripts/dev-pipeline-state.mjs init "<name>" "<source-branch>" --feature-id "<featureId>" --route "<route>"
   # 提供了 URL
   node <SKILL_ROOT>/scripts/dev-pipeline-state.mjs init "<name>" "<source-branch>" --feature-id "<featureId>" --feature-url "<featureUrl>" --route "<route>"
   ```
   其中 `<route>` 为 Step2 中用户选择的 Route 类型（`trivial` / `standard` / `full`）。
3. 用户选择 `跳过关联` 时执行：
   ```bash
   node <SKILL_ROOT>/scripts/dev-pipeline-state.mjs init "<name>" "<source-branch>" --skip-feature-association --route "<route>"
   ```
4. 上述 `init` 命令只能执行一个，不得传入 `<featureId>`、`<featureUrl>` 等占位值。后续每条状态命令都必须检查 exit code。

### 3.a 用户提供了已有 change 名称
...
1. 使用 **AskUserQuestion** 询问用户意图：
   ...
   - **options**：
     ...
     - `系分设计文档` — 输入已有细分的系分设计文档名称，以其内容作为需求描述创建 change
     - `查看已有 change 列表` — 列出所有可用 change，再决定续接或新建

2. 根据用户选择：
   - 选择 `已有 change 名称` → 发送文本消息请用户输入 change 名称，收到后走 **3.a** 路径
   - 选择 `新的需求描述` → 发送文本消息请用户描述需求，收到后走 **3.b** 路径
   - 选择 `系分设计文档` → 发送文本消息请用户输入已生成的系分设计文档名称（如 `openspec/docs/<yyyyMMdd>/<kebab-case-name>.md`），读取该文档内容作为需求描述，走 **3.b** 路径（从描述推导 change 名称、初始化状态并迁移到 Phase1）
   - 选择 `查看已有 change 列表` → 运行 `node <SKILL_ROOT>/scripts/list-changes.mjs`（或 `openspec list --json`）展示可用 change，展示后重新回到步骤 1（再次使用 **AskUserQuestion**，此时可额外追加当前已有的 change 名称作为附加选项供快速选择）
