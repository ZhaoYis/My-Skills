...
path_hints:
  ...
  - "src/**/*.test.*"
  - "src/**/*.spec.*"
description: "单元测试门禁的详细指引，包括测试命令识别、测试编写、结果记录"
---
...
## Step14：[决策点 4] 是否需要单元测试？

**AskUserQuestion**（不得默认跳过）：
- `需要：编写/补充单元测试并运行通过后再继续` → 执行子流程 A
- `不需要：跳过单测环节` → 进入 Phase5 Step15
...
3. 执行测试命令
4. 失败 → **AskUserQuestion**：`修复代码或测试后重试` / `跳过并记录技术债务（需写明原因和跟进 issue）` / `终止流程`
...
1. 在 `openspec/changes/<name>/` 下创建或追加 `tech-debt.md`，记录：
   ...
   - 跳过原因
   - 建议的跟进 issue / 负责人
2. 使用 **AskUserQuestion** 确认记录内容后继续 → 进入 Phase5 Step15
